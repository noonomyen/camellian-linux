...
- **`LU0`** (~128 GB) is the primary User Area containing the GPT table and
...
- **All partitions except `super` and `userdata`** (these two partitions are
  very large; `super` can be obtained directly from Fastboot/OTA packages,
  and `userdata` is automatically formatted and regenerated on first boot).
