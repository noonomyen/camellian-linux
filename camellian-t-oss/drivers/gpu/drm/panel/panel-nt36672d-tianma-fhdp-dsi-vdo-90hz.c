 ...
 * Function Prototype
 *****************************************************************************/
static int nova_i2c_probe(struct i2c_client *client,
...
//This function needs to be used in the main supply and second supply driver files
int _lcm_i2c_write_bytes(unsigned char addr, unsigned char value)
{
	...
	pr_info("prepare, i2c addr:%x\n", _lcm_i2c_client->addr);
 ...
 */
static int __init nova_i2c_init(void)
{
	pr_info("[LCM][I2C] %s\n", __func__);
...
static void __exit nova_i2c_exit(void)
{
	pr_debug("[LCM][I2C] %s\n", __func__);
	i2c_del_driver(&_lcm_i2c_driver);
}

module_init(nova_i2c_init);
module_exit(nova_i2c_exit);
/***********************************/
...
{
	pr_info("%s+\n", __func__);
	pr_err("[KERNEL][LCM]loading dsi_panel_K19_36_02_0c_vdo initial code...\n");
	// lcd reset H -> L -> L
	...
	if (esd_flag == true) {
		...
		tianma_dcs_write_seq_static(ctx, 0x4B, 0x0);
		msleep(1000);
	...
	tianma_dcs_write_seq_static(ctx, 0xFB, 0x01);
	tianma_dcs_write_seq_static(ctx, 0x85, 0x39);
...
static int nova_nova_tianma_disable(struct drm_panel *panel)
{
...
static int nova_tianma_unprepare(struct drm_panel *panel)
{
	struct tianma *ctx = panel_to_tianma(panel);
	...
	/* BSP.lcm - 2020.12.1 - pull down pwm pin */
	ctx->pwm =
...
static int tianma_prepare(struct drm_panel *panel)
{
	...
	/* BSP.lcm - 2020.12.1 - pull up pwm pin */
	ctx->pwm =
	    devm_gpiod_get(ctx->dev, "pwm", GPIOD_OUT_HIGH);
...
static int nova_tianma_enable(struct drm_panel *panel)
{
	struct tianma *ctx = panel_to_tianma(panel);
...
static int nova_panel_ata_check(struct drm_panel *panel)
{
	/* Customer test by own ATA tool */
	return 1;
}

static int nova_tianma_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle,
				 unsigned int level)
{
...
struct drm_display_mode *nova_get_mode_by_id_hfp(struct drm_panel *panel,
	unsigned int mode)
{
...
static int nova_mtk_panel_ext_param_set(struct drm_panel *panel, unsigned int mode)
{
	struct mtk_panel_ext *ext = find_panel_ext(panel);
	int ret = 0;
	struct drm_display_mode *m = nova_get_mode_by_id_hfp(panel, mode);
...
static int nova_mtk_panel_ext_param_get(struct mtk_panel_params *ext_para,
			 unsigned int mode)
{
	int ret = 0;
...
static void nova_mode_switch_to_90(struct drm_panel *panel)
{
	struct tianma *ctx = panel_to_tianma(panel);
...
static void nova_mode_switch_to_60(struct drm_panel *panel)
{
	struct tianma *ctx = panel_to_tianma(panel);
...
static int nova_mode_switch(struct drm_panel *panel, unsigned int cur_mode,
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (dst_mode == 60) { /* 60 switch to 120 */
		nova_mode_switch_to_60(panel);
	} else if (dst_mode == 90) { /* 1200 switch to 60 */
		nova_mode_switch_to_90(panel);
	} else
		ret = 1;
...
static int nova_panel_ext_reset(struct drm_panel *panel, int on)
{
	struct tianma *ctx = panel_to_tianma(panel);
...
static struct mtk_panel_funcs ext_funcs = {
	.reset = nova_panel_ext_reset,
	.set_backlight_cmdq = nova_tianma_setbacklight_cmdq,
	.ext_param_set = nova_mtk_panel_ext_param_set,
	.ext_param_get = nova_mtk_panel_ext_param_get,
	.mode_switch = nova_mode_switch,
	.ata_check = nova_panel_ata_check,
};
#endif
...
static int nova_tianma_get_modes(struct drm_panel *panel)
{
...
static const struct drm_panel_funcs tianma_drm_funcs = {
	.disable = nova_nova_tianma_disable,
	.unprepare = nova_tianma_unprepare,
	.prepare = tianma_prepare,
	.enable = nova_tianma_enable,
	.get_modes = nova_tianma_get_modes,
};

static int nova_tianma_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[LCM][KERNEL]dsi_panel_K19_36_02_0c_vdo probe!!!\n");
	if (strstr(saved_command_line, "dsi_panel_K19_36_02_0c_vdo")) {
		pr_err("dsi_panel_K19_36_02_0c_vdo probe match");
	} else {
		pr_err("not match dsi_panel_K19_36_02_0c_vdo !!!\n");
		return -ENODEV;
	}
	...
	pr_err("remote_node_name=%s, dev_of_node_name=%s\n", remote_node->name, dev->of_node->name);
	...
	pr_info("%s- dsi_panel_K19_36_02_0c_vdo,rt4801\n", __func__);

	return ret;
}

static int nova_tianma_remove(struct mipi_dsi_device *dsi)
{
...
static struct mipi_dsi_driver nova_tianma_driver = {
	.probe = nova_tianma_probe,
	.remove = nova_tianma_remove,
	.driver = {
		.name = "dsi_panel_K19_36_02_0c_vdo",
		.owner = THIS_MODULE,
		.of_match_table = tianma_of_match,
	},
};

module_mipi_dsi_driver(nova_tianma_driver);

MODULE_AUTHOR("Cui Zhang <cui.zhang@mediatek.com>");
MODULE_DESCRIPTION("tianma nt36672d VDO Panel Driver");
