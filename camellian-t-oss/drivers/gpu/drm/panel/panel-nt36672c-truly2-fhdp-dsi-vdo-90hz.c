...
static int truly2_disable(struct drm_panel *panel)
{
...
static int truly2_unprepare(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);
...
static int truly2_prepare(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);
	int ret;
...
static int truly2_enable(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);
...
static int truly2_panel_ata_check(struct drm_panel *panel)
{
	/* Customer test by own ATA tool */
	return 1;
}

static int truly2_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle,
				 unsigned int level)
{
...
struct drm_display_mode *truly2_get_mode_by_id_hfp(struct drm_panel *panel,
	unsigned int mode)
{
...
static int truly2_mtk_panel_ext_param_set(struct drm_panel *panel, unsigned int mode)
{
	struct mtk_panel_ext *ext = find_panel_ext(panel);
	int ret = 0;
	struct drm_display_mode *m = truly2_get_mode_by_id_hfp(panel, mode);
	printk("zzz+\n");
	if (m->vrefresh == 60) {
...
static int truly2_mtk_panel_ext_param_get(struct mtk_panel_params *ext_para,
			 unsigned int mode)
{
	int ret = 0;
...
static void truly2_mode_switch_to_90(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);
...
static void truly2_mode_switch_to_60(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);
...
static int truly2_mode_switch(struct drm_panel *panel, unsigned int cur_mode,
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (dst_mode == 60) { /* 60 switch to 120 */
		truly2_mode_switch_to_60(panel);
	} else if (dst_mode == 90) { /* 1200 switch to 60 */
		truly2_mode_switch_to_90(panel);
	} else
		ret = 1;
...
static int truly2_panel_ext_reset(struct drm_panel *panel, int on)
{
	struct truly *ctx = panel_to_truly(panel);
...
static struct mtk_panel_funcs ext_funcs = {
	.reset = truly2_panel_ext_reset,
	.set_backlight_cmdq = truly2_setbacklight_cmdq,
	.ext_param_set = truly2_mtk_panel_ext_param_set,
	.ext_param_get = truly2_mtk_panel_ext_param_get,
	.mode_switch = truly2_mode_switch,
	.ata_check = truly2_panel_ata_check,
};
#endif
...
static int truly2_get_modes(struct drm_panel *panel)
{
...
static const struct drm_panel_funcs truly_drm_funcs = {
	.disable = truly2_disable,
	.unprepare = truly2_unprepare,
	.prepare = truly2_prepare,
	.enable = truly2_enable,
	.get_modes = truly2_get_modes,
};

static int truly2_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[LCM][KERNEL]dsi_panel_K19_43_02_0b2_vdo probe!!!\n");

	if (strstr(saved_command_line, "dsi_panel_K19_43_02_0b2_vdo")) {
		pr_err("dsi_panel_K19_43_02_0b2_vdo probe match");
	} else {
		pr_err("not match dsi_panel_K19_43_02_0b2_vdo !!!\n");
		return -ENODEV;
	}
	...
	pr_info("%s- dsi_panel_K19_43_02_0b2_vdo,rt4801\n", __func__);

	return ret;
}

static int truly2_remove(struct mipi_dsi_device *dsi)
{
...
static const struct of_device_id truly2_of_match[] = {
	{
	    .compatible = "dsi_panel_K19_43_02_0b2_vdo",
	},
	{}
};

MODULE_DEVICE_TABLE(of, truly2_of_match);

static struct mipi_dsi_driver truly2_driver = {
	.probe = truly2_probe,
	.remove = truly2_remove,
	.driver = {
		.name = "dsi_panel_K19_43_02_0b2_vdo",
		.owner = THIS_MODULE,
		.of_match_table = truly2_of_match,
	},
};

module_mipi_dsi_driver(truly2_driver);

MODULE_AUTHOR("Cui Zhang <cui.zhang@mediatek.com>");
MODULE_DESCRIPTION("truly2 nt36672c VDO Panel Driver");
