...
static int gc02m1b_read_otp(void)
{
	...
	if (vendor_info_result[0] == FLAG_VALID) {
		...
		LOG_INF("%s: no valid vendor id\n", __func__);
		return -1;
	}

	if (vendor_id != QT_VENDOR_ID) {
		LOG_INF("%s: read vendorid(%d), QT vendor id(%d)\n", __func__, vendor_id, QT_VENDOR_ID);
		return -1;
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (ret != 0) {
		LOG_INF("Read gc02m1b  qtch otp info failed");
