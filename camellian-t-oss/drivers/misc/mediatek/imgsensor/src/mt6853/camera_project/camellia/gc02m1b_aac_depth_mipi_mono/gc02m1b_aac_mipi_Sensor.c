...
static kal_int8 gc02m1b_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
{
	int i = 0;

	if (addr == NULL || addr_len == 0 || result == NULL) {
		LOG_INF("Error info\n");
		return -1;
	}
	write_cmos_sensor(0xfe, 0x00);
	...
	for (i = 0; i < addr_len; i++) {
		write_cmos_sensor(0x17, addr[i]);
		write_cmos_sensor(0xf3, 0x34);
		result[i] = read_cmos_sensor(0x19);
	}
...
#define GROUP1_FLAG_ADDR 0x78
#define GROUP1_VENDOR_ADDR 0x80
#define GROUP2_FLAG_ADDR 0xB8
#define GROUP2_VENDOR_ADDR 0xC0
#define FLAG_VALID	0x01
#define AAC_VENDOR_ID 0x10

static int gc02m1b_read_otp(void)
{
	int ret = 0;
	kal_uint8 vendor_id = 0;
	int vendor_info_addr[] = {GROUP1_FLAG_ADDR, GROUP1_VENDOR_ADDR, GROUP2_FLAG_ADDR, GROUP2_VENDOR_ADDR};
	int fuseid_info_addr[] = {0x18, 0x20, 0x28, 0x30, 0x38, 0x40, 0x48, 0x50, 0x58};
	kal_uint8 vendor_info_result[ARRAY_SIZE(vendor_info_addr)] = {0};
	kal_uint8 fuseid_info_result[ARRAY_SIZE(fuseid_info_addr)] = {0};

	//read vendor id
	ret = gc02m1b_read_general_info(vendor_info_addr, ARRAY_SIZE(vendor_info_addr), vendor_info_result);
	if (ret != 0) {
		LOG_INF("%s: read vendor info failed\n", __func__);
		return ret;
	}

	if (vendor_info_result[0] == FLAG_VALID) {
		LOG_INF("%s: group 1 valid", __func__);
		vendor_id = vendor_info_result[1];
	} else if (vendor_info_result[2] == FLAG_VALID) {
		LOG_INF("%s: group 2 valid", __func__);
		vendor_id = vendor_info_result[3];
	} else {
		LOG_INF("%s: no valid vendor id, group 1 = %d, group 2 = %d\n", __func__, vendor_info_result[0], vendor_info_result[2]);
		return -1;
	}

	if (vendor_id != AAC_VENDOR_ID) {
		LOG_INF("%s: read vendorid(%d), QT vendor id(%d)\n", __func__, vendor_id, AAC_VENDOR_ID);
		return -1;
	}

	//read fuseid info
	ret = gc02m1b_read_general_info(fuseid_info_addr, ARRAY_SIZE(fuseid_info_addr), fuseid_info_result);
	if (ret != 0) {
		LOG_INF("%s: read fuseid info failed\n", __func__);
		return ret;
	}
	otp_data[0] = vendor_id;
	memcpy(otp_data + 1, fuseid_info_result, ARRAY_SIZE(otp_data) - 1);
	return ret;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	ret  = gc02m1b_read_otp();
	if (ret != 0) {
		LOG_INF("Read gc02m1b aac otp info failed");
