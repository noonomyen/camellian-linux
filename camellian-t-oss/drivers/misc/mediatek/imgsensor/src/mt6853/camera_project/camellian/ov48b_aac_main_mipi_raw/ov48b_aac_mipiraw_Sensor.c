...
#include "ov48b_aac_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV48B_AAC_MAIN_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("OV48B AAC get_imgsensor_id in\n");
	vendor_id = get_vendor_id();
	LOG_INF("%s: vendor_id: 0x%x,\n", __func__, vendor_id);
	if (vendor_id != 0x10) {
		LOG_INF("%s: vendor_id(0x%x) is not same with 0x10\n", __func__, vendor_id);
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		case MSDK_SCENARIO_ID_CUSTOM4:
		default:
			*feature_return_para_32 = 2000; /*BINNING_AVERAGED*/
			break;
		}
...
{
    LOG_INF("ov48b_aac_main_mipi_raw_SensorInit in\n");
